...
export default function OrlyPage() {
  const { t } = useLanguage();

  const details = t('airports_page.orly.details') as { label: string; value: string; subtext: string }[];
  const whyPoints = t('airports_page.orly.why_points') as string[];
  const bookingSteps = t('airports_page.orly.booking_steps') as { step: string; title: string; desc: string }[];
  const terminalData = t('airports_page.orly.terminals') as { name: string; desc: string; price: string }[];
  const terminalsTitle = t('airports_page.orly.terminals_title');
  const terminalsSubtitle = t('airports_page.orly.terminals_subtitle');
      ...
      <main>
        ...
        <section className="relative min-h-[60vh] flex items-center justify-center overflow-hidden">
          ...
          <div className="container text-center text-white py-24 pt-36">
            ...
            >
              {t('airports_page.orly.price_badge')}
            </motion.div>
            <motion.h1
            ...
            >
              {(() => {
                const titleStr = t('airports_page.orly.hero_title') as string;
                const highlight = titleStr.includes('Orly') ? 'Orly' : null;
                if (highlight) {
            ...
            <p className="text-lg md:text-xl text-gray-300 max-w-2xl mx-auto">
              {t('airports_page.orly.hero_subtitle')}
            </p>
            <p className="mt-4 text-brand font-semibold text-base md:text-lg">
              {t('airports_page.orly.price_label')}
            </p>
          </div>
        </section>
        {/* Quick info cards */}
          ...
          <div className="grid md:grid-cols-1 items-start">
            ...
            <div>
              <motion.h2
                ...
                viewport={{ once: true }}
              >
                {t('airports_page.orly.why_title')}
              </motion.h2>
              ...
              {/* <div className="mt-8 p-5 rounded-xl bg-brand/10 border border-brand/20">
                <h3 className="font-bold text-ink mb-2 flex items-center gap-2">
                  ...
                  {t('airports_page.orly.pickup_title')}
                </h3>
                <p className="text-ink-softer text-sm">{t('airports_page.orly.pickup_text')}</p>
              </div> */}
            </div>
          </div>
        </section>
        ...
        <section className="section bg-base-light">
          <div className="container">
            <motion.h2
              ...
              viewport={{ once: true }}
            >
              {t('airports_page.orly.booking_title')}
            </motion.h2>
          ...
          <div className="container">
            <div className="text-center mb-12">
              <motion.h2
                ...
                viewport={{ once: true }}
              >
                {terminalsTitle.split('Orly').map((part: string, i: number) => (
                  <span key={i}>
                    {part}
                    {i === 0 && <span className="text-brand">Orly</span>}
                  </span>
                ))}
              </motion.h2>
              <motion.p
            ...
            <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
              {Array.isArray(terminalData) && terminalData.map((terminal, i) => {
                const isCurrent = i === 0; // Orly 1 is current for this example
                return (
                  <motion.div
                    key={i}
                  ...
                  >
                    ...
                    <div className="mt-auto">
                      ...
                      <Link
                        href={`/?pickup=Paris&dropoff=Orly ${terminal.name}&fare=${terminal.price}&luggage=1#reservation`}
                        className={`
        ...
        <section className="section container text-center">
          <div className="bg-brand/10 rounded-2xl md:rounded-3xl p-8 md:p-12 max-w-3xl mx-auto">
            <h2 className="heading-section mb-4">{t('airports_page.orly.hero_title')}</h2>
            <p className="text-ink-softer text-lg mb-8">{t('airports_page.orly.hero_subtitle')}</p>
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <Link
                href="/#reservation"
                className="btn btn-primary px-8 py-3 text-base shadow-lg hover:shadow-xl transform hover:-translate-y-1"
              >
                {t('airports_page.orly.cta_button')}
              </Link>
              <a
